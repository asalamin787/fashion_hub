            ...
            <div class="card border-0 shadow-lg overflow-hidden">
                <div class="card-body p-4 p-lg-5">
                    <div class="row g-4 align-items-start">
                        ...
                        <div class="col-lg-4">
                            <div class="card border-0 shadow-sm"><div class="card-body p-4"><h2 class="h5 mb-3">Order Summary</h2><div class="d-flex justify-content-between mb-2"><span class="text-muted">Subtotal</span><strong>${{ number_format((float) $order->subtotal, 2) }}</strong></div><div class="d-flex justify-content-between mb-2"><span class="text-muted">Shipping</span><strong>${{ number_format((float) $order->shipping_amount, 2) }}</strong></div><div class="d-flex justify-content-between mb-2"><span class="text-muted">Tax</span><strong>${{ number_format((float) $order->tax_amount, 2) }}</strong></div>@if ((float) $order->coupon_discount_amount > 0)<div class="d-flex justify-content-between mb-2"><span class="text-muted">Coupon Discount @if ($order->coupon_code)<small class="text-success">({{ $order->coupon_code }})</small>@endif</span><strong class="text-success">- ${{ number_format((float) $order->coupon_discount_amount, 2) }}</strong></div>@endif @if ((float) $order->first_order_discount_amount > 0)<div class="d-flex justify-content-between mb-3"><span class="text-muted">First Order Discount ({{ number_format((float) $order->first_order_discount_rate, 0) }}%)</span><strong class="text-success">- ${{ number_format((float) $order->first_order_discount_amount, 2) }}</strong></div>@endif <hr><div class="d-flex justify-content-between"><span class="fw-semibold">Total</span><strong>${{ number_format((float) $order->total_amount, 2) }}</strong></div><div class="d-grid gap-2 mt-4"><a href="{{ auth()->check() ? route('account.orders.show', $order->order_number) : route('order.confirmation', $order->order_number) }}" class="btn btn-dark rounded-pill">View Order</a><a href="{{ route('shop') }}" class="btn btn-outline-dark rounded-pill">Continue Shopping</a></div><p class="small text-muted mt-3 mb-0">A confirmation email has been sent to {{ $order->customer_email }}.</p></div></div>
                        </div>
                    </div>
                </div>
            </div>
